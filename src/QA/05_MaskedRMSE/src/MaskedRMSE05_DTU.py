...
class RMSE:
    ...
    def compute_avg_rmse(self, old_data: pandas.DataFrame, database_dirpath: Path, pred_videos_dirpath: Path,
                         pred_folder_name: str, downsampling_factor: int, mask_folder_name: str):
        """
        ...
        for i, frame_data in tqdm(self.frames_data.iterrows(), total=self.frames_data.shape[0], leave=self.verbose_log):
            ...
            if old_data is not None and old_data.loc[
                ...
                continue
            gt_frame_path = database_dirpath / f'all/DatabaseData/{scene_num:05}/rgb/{pred_frame_num:04}.png'
            pred_frame_path = pred_videos_dirpath / f'{scene_num:05}/{pred_folder_name}/{pred_frame_num:04}.png'
            mask_path = database_dirpath / f'all/DatabaseData/{scene_num:05}/{mask_folder_name}/{pred_frame_num:04}.png'
            if not (pred_frame_path.exists() and mask_path.exists()):
                continue
...
def start_qa(pred_videos_dirpath: Path, database_dirpath: Path, frames_datapath: Path, pred_folder_name: str,
    ...
    rmse_data_path = pred_videos_dirpath / f'QA_Scores/{pred_folder_name}/{this_metric_name}_FrameWise.csv'
    ...
    if pred_folder_name in qa_scores:
        if this_metric_name in qa_scores[pred_folder_name]:
            avg_rmse = qa_scores[pred_folder_name][this_metric_name]
            print(f'Average {this_metric_name}: {pred_videos_dirpath.as_posix()} - {pred_folder_name}: {avg_rmse}')
    ...
    avg_rmse, rmse_data = mse_computer.compute_avg_rmse(rmse_data, database_dirpath, pred_videos_dirpath,
                                                        pred_folder_name, downsampling_factor, mask_folder_name)
    qa_scores[pred_folder_name][this_metric_name] = avg_rmse
    print(f'Average {this_metric_name}: {pred_videos_dirpath.as_posix()} - {pred_folder_name}: {avg_rmse}')
...
def demo1():
    ...
    frames_data_path = database_dirpath / 'TrainTestSets/Set02/TestVideosData.csv'
    pred_folder_name = 'PredictedFrames'
    downsampling_factor = 1
    mask_folder_name = 'ObjectMasks'
    avg_rmse = start_qa(pred_videos_dirpath, database_dirpath, frames_data_path, pred_folder_name, downsampling_factor, 
                        mask_folder_name)
    return avg_rmse
...
def demo2(args: dict):
    ...
    avg_rmse = start_qa(pred_videos_dirpath, database_dirpath, frames_datapath, pred_folder_name, downsampling_factor,
                        mask_folder_name)
    return avg_rmse
