...
class RMSE:
    ...
    def compute_avg_rmse(self, old_data: pandas.DataFrame, database_dirpath: Path, pred_videos_dirpath: Path,
                         pred_folder_name: str, resolution_suffix: str, downsampling_factor: int):
        """
        ...
        for i, frame_data in tqdm(self.frames_data.iterrows(), total=self.frames_data.shape[0], leave=self.verbose_log):
            ...
            if old_data is not None and old_data.loc[
                ...
                continue
            gt_frame_path = database_dirpath / f'all/DatabaseData/{scene_name}/rgb{resolution_suffix}/{pred_frame_num:04}.png'
            pred_frame_path = pred_videos_dirpath / f'{scene_name}/{pred_folder_name}/{pred_frame_num:04}.png'
...
def start_qa(pred_videos_dirpath: Path, database_dirpath: Path, frames_datapath: Path, pred_folder_name: str, 
    ...
    avg_rmse, rmse_data = mse_computer.compute_avg_rmse(rmse_data, database_dirpath, pred_videos_dirpath,
                                                        pred_folder_name, resolution_suffix, downsampling_factor)
...
def demo1():
    ...
    avg_rmse = start_qa(pred_videos_dirpath, database_dirpath, frames_data_path, pred_folder_name, resolution_suffix, 
                        downsampling_factor)
    return avg_rmse
...
def demo2(args: dict):
    ...
    avg_rmse = start_qa(pred_videos_dirpath, database_dirpath, frames_datapath, pred_folder_name, resolution_suffix,
                        downsampling_factor)
    return avg_rmse
