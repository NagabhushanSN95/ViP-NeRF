...
# Creates train-test sets based on PixelNeRF test set, but multiple train frames. test frames are the train frames from Pixel-NeRF
...
def create_train_test_set(configs: dict):
    ...
    test_frame_nums = [25, 22, 28, 40, 44, 48, 0, 8, 13]
    train_frame_nums = [frame_num for frame_num in range(49) if frame_num not in test_frame_nums]
    val_frame_nums = test_frame_nums[:2]
